...
function ShopSidebar( props ) {
    const [ value, setValue ] = useState( { max: 10000, min: 0 } )
    const [ toggle, setToggle ] = useState( false );
    const sizes = [ "XS", "S", "M", "L", "XL", "XXL" ];
    const brands = props.productsData.brand;
    const categories = props.productsData.category;
    ...
    categories.map( ( item, index ) => {
        return categoryCountArr.push( getCountByCategory( props.productsData.product.slice( 0, 47 ), item.name ) )
    } );
                                    ...
                                    <div className="widget-body">
                                        <div className="filter-items">
                                            { brands.map( ( item, index ) => (
                                                <div className="filter-item" key={ `brand_${index}` }>
                                                    <div className="custom-control custom-checkbox">

                                                        <input type="checkbox"
                                                            className="custom-control-input"
                                                            id={ `brand-${index+1}` }
                                                            onClick={ ( e ) => props.toggleBrandFilter( item.name ) }
                                                            defaultChecked={ -1 < findIndex( filters[ 'brand' ],
                                                                filter => filter === item.name ) ? true : false }
                                                        />
                                    ...
                                    <div className="widget-body">
                                        <div className="filter-price">
                                            <div className="filter-price-text">
                                                Price Range:&nbsp;
                                                <span className="filter-price-range">Rs.{ value.min } - Rs.{ value.max }</span>
                                            </div>
